...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`📝 [Step 2] Posting changed quantities. Items: ${modifiedTemplate.length}`);
    ...
    if (!response.ok) {
      ...
      console.error(`❌ [Step 2] TPOS API error: ${response.status}`, errorText);
      throw new Error(`TPOS API error: ${response.status}`);
    }

    const data = await response.json();
    console.log(`✅ [Step 2] Changed quantities posted successfully. Returned IDs: ${data.value?.length || 0}`);
